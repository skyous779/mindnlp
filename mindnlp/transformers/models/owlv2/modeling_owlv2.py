...
class Owlv2VisionEmbeddings(nn.Cell):
    def __init__(self, config: Owlv2VisionConfig):
        ...
        self.class_embedding = mindspore.Parameter(ops.randn(config.hidden_size), 'class_embedding')
...
class Owlv2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, Owlv2TextEmbeddings):
            ...
            cell.patch_embedding.weight.data.initialize(Normal(cell.config.initializer_range * factor))
            cell.position_embedding.weight.data.initialize(Normal(cell.config.initializer_range * factor))
            ...
            cell.text_projection.weight.data.initialize(Normal(cell.text_embed_dim**-0.5 * self.config.initializer_factor))
            cell.visual_projection.weight.data.initialize(Normal(cell.vision_embed_dim**-0.5 * self.config.initializer_factor))
...
class Owlv2TextTransformer(nn.Cell):
    ...
    def construct(
        ...
        pooled_output = last_hidden_state[
            ...
            input_ids.astype(mindspore.int32).argmax(axis=-1),
        ]
...
class Owlv2Model(Owlv2PreTrainedModel):
    ...
    def __init__(self, config: Owlv2Config):
        ...
        self.logit_scale = mindspore.Parameter(mindspore.Tensor(config.logit_scale_init_value), 'logit_scale')
...
class Owlv2ForObjectDetection(Owlv2PreTrainedModel):
    ...
    def objectness_predictor(self, image_features: mindspore.Tensor) -> mindspore.Tensor:
        ...
        Returns:
            Objectness scores.
        """
        image_features.stop_gradient = True
    ...
    def image_guided_detection(
        ...
        batch_size, num_patches1, num_patches2, hidden_dim = feature_map.shape
        image_feats = ops.reshape(feature_map, (batch_size, num_patches1 * num_patches2, hidden_dim))

        batch_size_, num_patches_1, num_patches_2, hidden_dim_ = query_feature_map.shape
        query_image_feats = ops.reshape(query_feature_map, (batch_size_, num_patches_1 * num_patches_2, hidden_dim_))
    ...
    def construct(
        ...
        batch_size_1, num_patches1_, num_patches2_, hidden_dim_1 = feature_map.shape
        image_feats = ops.reshape(feature_map, (batch_size_1, num_patches1_ * num_patches2_, hidden_dim_1))

        # Reshape from [batch_size * max_text_queries, hidden_dim] -> [batch_size, max_text_queries, hidden_dim]
        max_text_queries = input_ids.shape[0] // batch_size_1
        query_embeds = query_embeds.reshape(batch_size_1, max_text_queries, query_embeds.shape[-1])

        # If first token is 0, then this is a padded query [batch_size, num_queries].
        input_ids = input_ids.reshape(batch_size_1, max_text_queries, input_ids.shape[-1])
